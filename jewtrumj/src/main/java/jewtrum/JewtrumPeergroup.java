...
import org.shekelj.core.CoinDefinition;
...
import jewtrum.exceptions.InvalidPeerVersion;
import jewtrum.listeners.AddressListener;
import jewtrum.listeners.PeerDataListener;
import jewtrum.listeners.PeerListener;
import jewtrum.messages.VersionMsg;
 ...
 *
 * Class in charge of manage the connection with jewtrum servers.
 *
...
public class JewtrumPeergroup implements PeerListener, PeerDataListener {

    private static final Logger log = LoggerFactory.getLogger(JewtrumPeergroup.class);
    ...
    /** Trusted peer */
    private JewtrumPeer trustedPeer;
    // Currently active peers.
    private final CopyOnWriteArrayList<JewtrumPeer> peers;
    // Currently connecting peers.
    private final CopyOnWriteArrayList<JewtrumPeer> pendingPeers;
    ...
    public JewtrumPeergroup(NetworkConf networkConf, WalletManager walletManager, AddressStore addressStore) throws IOException {
    ...
    public JewtrumPeergroup(NetworkConf networkConf) throws IOException {
        this.peers = new CopyOnWriteArrayList<>();
        this.pendingPeers = new CopyOnWriteArrayList<>();
        this.networkConf = networkConf;
        this.ioManager = new IoManager(1,1);
    ...
    public synchronized void start() throws InterruptedException, ConnectionFailureException {
        try {
            log.info("Starting JewtrumPeergroup");
            isActive = true;
            // todo: first part discovery..
            /*
            * Connect to the trusted node and get servers from it.
            */
            trustedPeer = new JewtrumPeer(networkConf.getTrustedServer(), ioManager,versionMsg);
            ...
            isRunning = false;
            isActive = false;
            log.error("JewtrumPeerGroup start",e);
            throw e;
        }
    }
    ...
    @Override
    public void onConnected(JewtrumPeer jewtrumPeer) {
        try {
            if (jewtrumPeer == trustedPeer) {
                ...
                for (PeerListener peerConnectionListener : peerConnectionListeners) {
                    peerConnectionListener.onConnected(jewtrumPeer);
                }
                ...
                for (InetSocketAddress inetSocketAddress : networkConf.getNetworkServers()) {
                    JewtrumPeerData peerData = new JewtrumPeerData(inetSocketAddress.getHostName(),inetSocketAddress.getPort(),0);
                    JewtrumPeer peer = new JewtrumPeer(peerData,ioManager,versionMsg);
                ...
                log.info("Non trusted peer connected, "+jewtrumPeer.getPeerData());
                pendingPeers.remove(jewtrumPeer);
                peers.add(jewtrumPeer);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public void onDisconnected(JewtrumPeer jewtrumPeer) {

    }

    @Override
    public void onExceptionCaught(JewtrumPeer jewtrumPeer, Exception e) {
    ...
    @Override
    public void onSubscribedAddressChange(JewtrumPeer jewtrumPeer, String address, String status) {
        try {
    ...
    @Override
    public void onListUnpent(JewtrumPeer jewtrumPeer,String address, List<Unspent> unspents) {
    ...
    @Override
    public void onBalanceReceive(JewtrumPeer jewtrumPeer, String address, long confirmed, long unconfirmed) {
        try {
            if (jewtrumPeer == trustedPeer) {
    ...
    @Override
    public void onGetHistory(JewtrumPeer jewtrumPeer, StatusHistory statusHistory) {
        try {
