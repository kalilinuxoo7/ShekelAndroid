package shekel.org.shekelwallet.rate.db;
...
import shekel.org.shekelwallet.contacts.AbstractSqliteDb;

/**
 * Created by kaali on 7/5/17.
 */

public class RateDb extends AbstractSqliteDb<ShekelRate> {
    ...
    @Override
    protected ContentValues buildContent(ShekelRate obj) {
    ...
    @Override
    protected ShekelRate buildFrom(Cursor cursor) {
        ...
        return new ShekelRate(coin,value,timestap,link);
    }
    ...
    public void insertOrUpdateIfExist(ShekelRate shekelRate) {
        if (getRate(shekelRate.getCoin())==null){
            insert(shekelRate);
        }else {
            updateByKey(KEY_COIN,shekelRate.getCoin(),shekelRate);
        }
    }
}
