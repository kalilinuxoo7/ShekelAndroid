package shekel.org.shekelwallet.service;
...
import org.shekelj.core.Block;
import org.shekelj.core.Coin;
import org.shekelj.core.FilteredBlock;
import org.shekelj.core.Peer;
import org.shekelj.core.Transaction;
import org.shekelj.core.TransactionConfidence;
import org.shekelj.core.listeners.AbstractPeerDataEventListener;
import org.shekelj.core.listeners.PeerConnectedEventListener;
import org.shekelj.core.listeners.PeerDataEventListener;
import org.shekelj.core.listeners.PeerDisconnectedEventListener;
import org.shekelj.core.listeners.TransactionConfidenceEventListener;
import org.shekelj.store.BlockStore;
import org.shekelj.wallet.Wallet;
import org.shekelj.wallet.listeners.WalletCoinsReceivedEventListener;
...
import shekel.org.shekelwallet.module.ShekelModuleImp;
import shekel.org.shekelwallet.module.store.SnappyBlockchainStore;
import shekel.org.shekelwallet.rate.CoinMarketCapApiClient;
import shekel.org.shekelwallet.rate.CoinTypes;
import shekel.org.shekelwallet.rate.RequestShekelRateException;
...
import static shekel.org.shekelwallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static shekel.org.shekelwallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static shekel.org.shekelwallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
import static shekel.org.shekelwallet.service.IntentsConstants.ACTION_NOTIFICATION;
import static shekel.org.shekelwallet.service.IntentsConstants.ACTION_RESET_BLOCKCHAIN;
import static shekel.org.shekelwallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
import static shekel.org.shekelwallet.service.IntentsConstants.DATA_TRANSACTION_HASH;
import static shekel.org.shekelwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_BLOCKCHAIN_STATE;
import static shekel.org.shekelwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_ON_COIN_RECEIVED;
import static shekel.org.shekelwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_PEER_CONNECTED;
import static shekel.org.shekelwallet.service.IntentsConstants.INTENT_BROADCAST_DATA_TYPE;
import static shekel.org.shekelwallet.service.IntentsConstants.INTENT_EXTRA_BLOCKCHAIN_STATE;
import static shekel.org.shekelwallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static shekel.org.shekelwallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by kaali on 6/12/17.
 */

public class ShekelWalletService extends Service{

    private Logger log = LoggerFactory.getLogger(ShekelWalletService.class);

    private ShekelApplication shekelApplication;
    private ShekelModuleImp module;
    //private JewtrumPeergroup jewtrumPeergroup;
    ...
    public class ShekelBinder extends Binder {
        public ShekelWalletService getService() {
            return ShekelWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (ShekelContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    shekelApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.shekelj.core.Context.propagate(ShekelContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.shekelj.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), ShekelWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(ShekelWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                    mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            .setContentTitle("JEW received!")
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(ShekelWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("Shekel service started");
            // Android stuff
            ...
            module = (ShekelModuleImp) shekelApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to jewtrum node
            /*jewtrumPeergroup = new JewtrumPeergroup(shekelApplication.getNetworkConf());
            jewtrumPeergroup.addAddressListener(addressListener);
            module.setJewtrumPeergroup(jewtrumPeergroup);*/
            ...
            String filename = ShekelContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(ShekelContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("Shekel service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    public void onDestroy() {
        ...
        try {
            ...
            /*if (jewtrumPeergroup.isRunning()) {
                jewtrumPeergroup.shutdown();
            }*/
    ...
    private void tryScheduleService() {
        ...
        if (!isSchedule){
            ...
            Intent intent = new Intent(this, ShekelWalletService.class);
    ...
    private void requestRateCoin(){
        final AppConf appConf = shekelApplication.getAppConf();
        ShekelRate shekelRate = module.getRate(appConf.getSelectedRateCoin());
        if (shekelRate==null || shekelRate.getTimestamp()+ShekelContext.RATE_UPDATE_TIME<System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        BigDecimal usdValue = c.getShekelPrice();
                        ShekelRate shekelRate = new ShekelRate(CoinTypes.USD.name(),usdValue,System.currentTimeMillis(),"coinmarketcap");
                        module.saveRate(shekelRate);
                    } catch (RequestShekelRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
