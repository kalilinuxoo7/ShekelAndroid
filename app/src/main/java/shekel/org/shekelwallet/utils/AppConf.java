...
import static shekel.org.shekelwallet.module.ShekelContext.DEFAULT_RATE_COIN;
...
public class AppConf extends Configurations {
    ...
    public void saveTrustedNode(JewtrumPeerData jewtrumPeerData){
        save(TRUSTED_NODE_HOST,jewtrumPeerData.getHost());
        save(TRUSTED_NODE_TCP,jewtrumPeerData.getTcpPort());
        save(TRUSTED_NODE_SSL,jewtrumPeerData.getSslPort());
    }
    public JewtrumPeerData getTrustedNode(){
        String host = getString(TRUSTED_NODE_HOST,null);
        if (host!=null){
            int tcp = getInt(TRUSTED_NODE_TCP,-1);
            int ssl = getInt(TRUSTED_NODE_TCP,-1);
            return new JewtrumPeerData(host,tcp,ssl);
        }else
            return null;
    }
