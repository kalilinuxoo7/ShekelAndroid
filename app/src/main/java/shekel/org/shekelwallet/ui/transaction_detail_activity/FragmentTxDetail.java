...
import org.shekelj.core.TransactionOutPoint;
...
import shekel.org.shekelwallet.ui.base.BaseFragment;
import shekel.org.shekelwallet.ui.base.tools.adapter.BaseRecyclerAdapter;
import shekel.org.shekelwallet.ui.base.tools.adapter.BaseRecyclerViewHolder;
import shekel.org.shekelwallet.ui.base.tools.adapter.ListItemListeners;
...
import static shekel.org.shekelwallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (intent!=null){
            ...
            if (intent.hasExtra(IS_DETAIL)){
                transactionWrapper.setTransaction(shekelModule.getTx(transactionWrapper.getTxId()));
                isTxDetail = true;
            }else {
                transactionWrapper.setTransaction(new Transaction(shekelModule.getConf().getNetworkParams(),intent.getByteArrayExtra(TX)));
        ...
        if (transactionWrapper.isStake()){
            ...
            try {
                ...
                for (TransactionInput input : transactionWrapper.getTransaction().getInputs()) {
                    ...
                    inputsSum = inputsSum.plus(shekelModule.getUnspentValue(unspent.getHash(), (int) unspent.getIndex()));
                }
            ...
            if (transactionWrapper.getOutputLabels()!=null && transactionWrapper.getOutputLabels().containsKey(transactionOutput.getIndex())){
                ...
                if (addressLabel !=null) {
                    if (addressLabel.getName() != null) {
                        ...
                        label = transactionOutput.getScriptPubKey().getToAddress(shekelModule.getConf().getNetworkParams(),true).toBase58();
                }else {
                    label = transactionOutput.getScriptPubKey().getToAddress(shekelModule.getConf().getNetworkParams(),true).toBase58();
                }
            }else {
                ...
                if (script.isPayToScriptHash() || script.isSentToRawPubKey() || script.isSentToAddress()) {
                    label = script.getToAddress(shekelModule.getConf().getNetworkParams(), true).toBase58();
                }else {
    ...
    private void setupOutputs(List<OutputUtil> list) {
        ...
        ListItemListeners<OutputUtil> listItemListener = new ListItemListeners<OutputUtil>() {
            ...
            public void onLongItemClickListener(OutputUtil data, int position) {
                if (shekelModule.chechAddress(data.getLabel())) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.txt_inputs){
            try {
                ...
                bundle.putSerializable(INTENT_EXTRA_UNSPENT_WRAPPERS, (Serializable) shekelModule.convertFrom(transactionWrapper.getTransaction().getInputs()));
