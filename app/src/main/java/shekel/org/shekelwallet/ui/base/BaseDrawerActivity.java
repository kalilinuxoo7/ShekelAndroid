...
import shekel.org.shekelwallet.ui.contacts_activity.ContactsActivity;
import shekel.org.shekelwallet.ui.donate.DonateActivity;
import shekel.org.shekelwallet.ui.settings_activity.SettingsActivity;
import shekel.org.shekelwallet.ui.wallet_activity.WalletActivity;

import static shekel.org.shekelwallet.module.ShekelContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends ShekelActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = shekelApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = shekelModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (shekelModule.getChainHeight()+10>peerHeight) {
            ...
            if (shekelModule.isAnyPeerConnected()) {
                long peerHeight = shekelModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (shekelModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    private double calculateBlockchainSyncProgress() {
        long nodeHeight = shekelModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (shekelModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
