package shekel.org.shekelwallet.ui.pincode_activity;
...
import shekel.org.shekelwallet.ui.backup_mnemonic_activity.MnemonicActivity;
import shekel.org.shekelwallet.ui.base.BaseActivity;
import shekel.org.shekelwallet.ui.settings_pincode_activity.KeyboardFragment;
import shekel.org.shekelwallet.ui.start_activity.StartActivity;

import static shekel.org.shekelwallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (shekelApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (shekelApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<JewtrumPeerData> nodes = JewtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            shekelApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
            shekelApplication.stopBlockchain();
        }

        shekelApplication.getAppConf().setAppInit(true);
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        shekelApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(shekelApplication.getAppConf().getPincode().equals(pincode)){
