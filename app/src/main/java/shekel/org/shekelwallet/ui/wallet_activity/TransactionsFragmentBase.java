...
import shekel.org.shekelwallet.ui.base.BaseRecyclerFragment;
...
import shekel.org.shekelwallet.ui.transaction_detail_activity.TransactionDetailActivity;

import static shekel.org.shekelwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static shekel.org.shekelwallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static shekel.org.shekelwallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = shekelModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (shekelRate!=null) {
                    localCurrency = shekelApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * shekelRate.getValue().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + shekelRate.getCoin();
                ...
                holder.title.setText(getAddressOrContact(shekelModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(shekelModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(shekelModule.getConf().getNetworkParams()).toBase58());
                }*/
